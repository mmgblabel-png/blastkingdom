...
namespace BlockPuzzleGameToolkit.Scripts.Services.Ads.Networks
{
    ...
    public class AdmobHandler : AdsHandlerBase
    {
        ...
        {
#if UMP_AVAILABLE
            // Check if we can request personalized ads
            if (ConsentInformation.CanRequestAds())
            {
                Debug.Log("Creating personalized ad request");
                return new AdRequest();
            }
            else
            {
                Debug.Log("Creating non-personalized ad request");
                // For non-personalized ads, we still create a regular AdRequest
                // The UMP SDK handles the consent status automatically
                return new AdRequest();
            }
#else
            return new AdRequest();
