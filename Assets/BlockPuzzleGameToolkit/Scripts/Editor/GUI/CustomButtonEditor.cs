    ...
    internal class CustomButtonDrawer : UnityEditor.Editor
    {
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            isRewardedField.RegisterValueChangeCallback(evt =>
            {
                ...
                {
                    adReferenceProperty.objectReferenceValue = AssetDatabase.LoadAssetAtPath<AdReference>("Assets/BlockPuzzleGameToolkit/Prefabs/ScriptableAds/AdsTypes/Rewarded.asset");
                    serializedObject.ApplyModifiedProperties();
                }
            });
            root.Add(adReferenceField);

            var overrideClickSoundField = new PropertyField(serializedObject.FindProperty("overrideClickSound"));
            overrideClickSoundField.Bind(serializedObject);
            root.Add(overrideClickSoundField);

            
            var overrideAnimatorControllerField = new PropertyField(serializedObject.FindProperty("overrideAnimatorController"));
            overrideAnimatorControllerField.Bind(serializedObject);
            root.Add(overrideAnimatorControllerField);
