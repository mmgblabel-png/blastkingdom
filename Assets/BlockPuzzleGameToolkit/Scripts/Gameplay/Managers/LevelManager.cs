    ...
    public partial class LevelManager : MonoBehaviour
    {
        ...
        private void RestoreGameState()
        {
            ...
            if (state != null)
            {
                ...
                {
                    classicModeHandler.score = state.score;
                    classicModeHandler.bestScore = Mathf.Max(
                        classicModeHandler.bestScore,
                        Mathf.Max(state.bestScore, GetClassicBestScore()));
                    classicModeHandler.scoreText.text = state.score.ToString();
                    classicModeHandler.bestScoreText.text = classicModeHandler.bestScore.ToString();
                }
        ...
        private void RestartLevel()
        {
            ...
            if (classicModeHandler != null)
            {
                classicModeHandler.ResetScore();
            }

            if (timedModeHandler != null)
            {
                timedModeHandler.ResetScore();
            }

            // Don't reset GameManager.instance.Score here as it's used for best score storage
            // The mode handlers will properly handle current score reset
        ...
        private void SaveGameState()
        {
            if (gameMode == EGameMode.Classic)
            {
                ...
                var state = new ClassicGameState
                {
                    ...
                    bestScore = Mathf.Max(classicModeHandler.bestScore, GetClassicBestScore()),
        ...
        private void EnsureDeckHasShapes()
        {
            if (cellDeck == null || GameManager.instance.IsTutorialMode())
            {
                return;
            }

            if (cellDeck.GetShapes().Length == 0)
            {
                cellDeck.FillCellDecks();
            }
        }

        private int GetClassicBestScore()
        {
